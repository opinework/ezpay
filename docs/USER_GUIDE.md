...
> 提现使用 **卖出汇率**，平台按优惠汇率转换支付给商户。
> 例如：商户申请提现 100 USD，卖出浮动 -2%，实际打款 USDT = 100 × (1-0.02) = 98 USDT
...
系统采用 **USD 统一结算**，所有订单按 USD 结算，支持多币种支付。

| 设置项 | 说明 |
|-------|------|
| 自动更新 | 是否启用汇率自动更新 |
| 更新间隔 | 汇率更新间隔（分钟），默认60 |
| 主数据源 | 主要汇率数据源: binance/okx |
| 备用数据源 | 主数据源失败时使用的备用源 |
| 买入浮动 | 用户支付时汇率上浮（如 +2%），平台多收 |
| 卖出浮动 | 商户提现时汇率下浮（如 -2%），平台少给 |

**买卖价差利润**：买入浮动 + 卖出浮动 = 汇差收益（如 2% + 2% = 4%）
...
**汇率数据源**：
- Binance API (EUR/USD, TRX/USDT)
- exchangerate-api.com (USD/CNY)
- OKX API (备用)

> 所有汇率自动从交易所实时获取，支持手动调整或指定模式。
...
| telegram_admin_chat_id | 管理员群组 Chat ID (可选) |
...
**账号状态管理**：
- 系统自动检测 Telegram 账号封禁状态
- 封禁后自动停止推送，避免无效通知消耗
- 状态包括：normal (正常)、blocked (被封禁)、unbound (未绑定)
...
- 手续费：1 USD
- 实际金额：99 USD
- 卖出浮动 -2%：99 × (1-0.02) = 97.02 USDT 打款
...
**货币转换示例**：
- 商品标价：100 EUR
- 买入汇率：1 EUR = 1.08 USD × 1.02 (买入浮动)
- 结算金额：110.16 USD (计入商户余额)
- 实际支付：112.41 USDT (用户支付金额)
